...
def suggest_justification_text(
    ...
    *,
    ...
    return _complete(
        llm_client, messages, model=model, max_tokens=64, temperature=0.3, timeout=timeout
    ).strip()
...
def suggest_refined_query_text(
    ...
    *,
    ...
    messages = [{"role": "system", "content": system}, {"role": "user", "content": user}]
    return _complete(
        llm_client, messages, model=model, max_tokens=128, temperature=0.3, timeout=timeout
    ).strip()
